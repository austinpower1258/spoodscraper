...
async function processPage(url: string, page: Page, index: number, imageOffset: number): Promise<number> {
  ...
  const downloadAndProcessImage = async (imageUrl: string, i: number) => {
    ...
    console.log(sizeInKB, imageUrl);

    if (sizeInKB < 12 || sizeInKB >= 65) {
      console.log(`Skipping bad images: ${imageUrl}`);
      return;
    }
    ...
    if (convertedImage) {
      console.log(convertedImage.length / 1024, imageUrl);
      if (convertedImage.length / 1024 < 20 || convertedImage.length / 1024 >= 65) {
        console.log(`Skipping bad images: ${imageUrl}`);
        return;
      }
...
async function main() {
  ...
  const urls = [
    ...
    // "https://www.yelp.com/biz_photos/south-bay-new-haven-3?tab=food",
    // "https://www.yelp.com/biz_photos/prime-16-new-haven?tab=food"
  ];
